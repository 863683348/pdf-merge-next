...
interface AppState {
  ...
  restoreSubscription: (sub: string) => Promise<void>;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  logout: () => {
    clearStoredUser();
    writeStoredSub(null); // 同步清空订阅，避免下一位用户看到上一位的 Pro 状态
    set({ gaUser: null, subscription: null });
    try {
  ...
  // 登录后/初始化时调 /api/user/subscription 恢复 Pro（DB 为权威源）
  // - DB 有 active 订阅 → 写 localStorage + set subscription（跨设备恢复 Pro）
  // - DB 无订阅 → 清 localStorage（以 DB 为准，解决"换设备残留/已取消仍 Pro"）
  // - DB 降级（未配 DATABASE_URL）→ 保留 localStorage，不阻塞用户
  // - 网络错误 → 保留 localStorage（离线兼容）
  restoreSubscription: async (sub) => {
    try {
      const res = await fetch(
        `/api/user/subscription?sub=${encodeURIComponent(sub)}`
      );
      if (!res.ok) return; // 网络错误，保留 localStorage
      const data = (await res.json()) as {
        ok: boolean;
        plan?: string | null;
        degraded?: boolean;
      };
      if (data.degraded) return; // DB 未配，保留 localStorage
      if (data.plan === 'pro_monthly' || data.plan === 'pro_yearly') {
        get().setSubscription({
          plan: data.plan,
          since: new Date().toISOString(),
        });
      } else {
        // DB 明确无订阅 → 清 localStorage（DB 权威）
        get().clearSubscription();
      }
    } catch {
      /* 网络错误，保留 localStorage */
    }
  },
}));
