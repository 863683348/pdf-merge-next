...
export function GoogleLoginButton() {
  ...
  useEffect(() => {
    ...
    if (stored && !gaUser) {
      login(stored);
      // 从 DB 恢复订阅状态（DB 权威，覆盖 localStorage 残留 / 跨设备同步）
      void useAppStore.getState().restoreSubscription(stored.sub);
    }
    ...
    const tryInit = () => {
      ...
      tokenClientRef.current = oauth2.initTokenClient({
        ...
        callback: async (response: { access_token?: string; error?: string }) => {
          ...
          if (!user) return;
          login(user);
          // 注册到 DB（不阻塞 UI，失败静默；access_token 一次性用完即弃）
          void fetch('/api/user/register', {
            method: 'POST',
          ...
          // 从 DB 恢复订阅状态（跨设备/清缓存后恢复 Pro）
          void useAppStore.getState().restoreSubscription(user.sub);
        },
      });
      setGsiReady(true);
      return true;
  ...
  return (
    <button
      ...
      disabled={!gsiReady}
      className="inline-flex h-9 items-center gap-2 whitespace-nowrap rounded-md border border-line bg-surface px-3 text-sm font-medium text-fg-secondary transition-colors duration-fast hover:bg-subtle hover:text-fg disabled:cursor-wait disabled:opacity-50"
    >
      {/* SVG Google G logo */}
